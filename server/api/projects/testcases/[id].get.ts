...
/**
 * @brief Fetches evaluation statistics for test cases associated with a project.
 *
 * @param event H3 event object containing the project ID in the URL parameters.
 *
 * @returns {Promise<object>} An object containing test case data, total count, and chart data.
 *
...
export default defineEventHandler(async (event): Promise<object> => {
  try {
    ...
    consola.info(`Fetching test case evaluations for project with ID: ${id}`)
    ...
    const chartData = [...new Set(chartEval.map(e => e.checkitem?.module))].map((module) => {
      return {
        "name": module,
        "OK": chartEval.filter(
          e => e.checkitem?.module === module && e.judgement === EVALUATION_JUDGEMENT.OK,
        ).length,
        "NG": chartEval.filter(
          e => e.checkitem?.module === module && e.judgement === EVALUATION_JUDGEMENT.NG,
        ).length,
        "Not Executed": chartEval.filter(
          e => e.checkitem?.module === module && e.judgement === EVALUATION_JUDGEMENT.NOT_EXECUTED,
        ).length,
      }
    })
    ...
    consola.error(`Error fetching test case evaluations: ${(error as Error).message}`)

    // Return a generic error response
    throw new Error("An error occurred while fetching test case evaluations")
  }
})
