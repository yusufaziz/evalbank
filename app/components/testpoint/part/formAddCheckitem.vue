...
import type { IGroupedSettings, ISelectedSetting } from "~~/shared/interface/settings"

const model = defineModel<Checkitem[]>({
  default: () => [],
})

const newCheckitem = ref<ICheckitem>({
  id: "-",
  module: "Unknown",
  expectedTarget: "No target described",
  settings: [], // Replaced requiredSettings with settings
})
...
watch(selectedSettings, () => {
  newCheckitem.value.settings = selectedSettings.value.map(item => ({
    name: item.setting,
    value: item.additionalData.join(","), // Store values as a comma-separated string
  }))
...
function addNewCheckitem() {
  model.value.push({ ...newCheckitem.value } as Checkitem)
  initializeCheckitem()
...
function updateCheckitem() {
  if (editedCheckitem.value) {
    ...
    if (index !== -1) {
      model.value[index] = { ...newCheckitem.value } as Checkitem
    }
  }
                ...
                <form id="formCheckitem" @submit.prevent="editedCheckitem ? updateCheckitem() : addNewCheckitem()">
                  <fieldset class="grid gap-3">
                    <div class="flex flex-row gap-3">
                      <UiVeeInput v-model="newCheckitem.module" label="Module" class="w-200 text-sm" />
                      <UiVeeInput v-model="newCheckitem.expectedTarget" label="Expected Target" class="text-sm" />
                    </div>
                    <div>
                      <label class="block text-sm font-medium text-gray-700 mb-1">Required Settings</label>
                      <TestpointPartSettingSelection
                        v-model:selected-settings="selectedSettings"
                      />
                    </div>
                  </fieldset>
                </form>
