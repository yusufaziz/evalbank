...
/**
 * @brief Seeds the database with initial data for settings, projects, test cases, and check items.
 */
async function main() {
  try {
    // Create 100 Settings
    ...
    const printingOptions: PrintingOptions = {
      "Paper Type": [
        ...
        "PLPP",
        "LCPP",
        "Double-sided business card paper (Semi glossy)",
        "Glossy Postcard",
    ...
    for (const key in printingOptions) {
      if (Object.hasOwn(printingOptions, key)) {
    ...
    const testcaseSettingIds = await prisma.setting.findMany({
      where: {
        OR: [{ name: "Paper Type" }, { name: "Paper Size" }, { name: "Printing Quality" }],
      },
      select: { id: true },
    })
    ...
    const testcaseSettingIds2 = await prisma.setting.findMany({
      where: {
        OR: [{ name: "Paper Type" }, { name: "Printing Quality" }, { name: "SKU" }],
      },
      select: { id: true },
    })
    ...
    const testcaseSettingIds3 = await prisma.setting.findMany({
      where: {
        OR: [{ name: "Printing Quality" }, { name: "SKU" }, { name: "Copy Mode" }],
      },
      select: { id: true },
    })
    ...
    consola.error("Error during seeding:", error)
    process.exit(1)
  }
  finally {
